...
export class Room extends colyseus.Room {
    constructor() {
        super(...arguments);
        this.name = "CardsGame test";
    }
    onInit(options) {
    ...
    setupPrivatePropsSync() {
        this.state.on(EntityEvents.privateAttributeChange, (data) => {
    ...
    removePlayer(clientID) {
        ...
        if (!player) {
            logs.error("removePlayer", `can't find player's data, removed already?`, clientID);
        }
    ...
    onMessage(client, event) {
        ...
        event.player = this.state.players
            .toArray()
            .find(playerData => playerData.clientID === client.id).entity;
        const minifyTarget = (e) => {
            return `${e.type}:${e.name}`;
        };
        const minifyPlayer = (p) => {
            return `${p.type}:${p.name}[${p.clientID}]`;
        };
        logs.info("onMessage", JSON.stringify(Object.assign({ ...event }, event.target ? { target: minifyTarget(event.target) } : {}, { targets: event.targets.map(minifyTarget) }, event.player ? { player: minifyPlayer(event.player) } : {})));
        this.performAction(client, event);
    }
    /**
     * Check conditions and perform given action
     */
    performAction(client, event) {
        ...
        logs.info("performAction", `only ${actions.length} filtered by conditions:`, logActions);
        if (actions.length > 1) {
            logs.warn("performAction", `Whoops, even after filtering actions by conditions, I still have ${actions.length} actions!`);
            // log(actions)
        }
        if (actions.length === 0) {
        ...
        const result = this.commandsManager.orderExecution(actions[0].commandFactory, this.state, event);
        if (result) {
     ...
     */
    getActionsByInteraction(event) {
        const possibleEntityProps = ["name", "type", "value", "rank", "suit"];
        const actions = Array.from(this.possibleActions.values()).filter(template => {
             ...
             */
            const entityMatchesInteraction = (target, interaction) => {
                return possibleEntityProps.every((prop) => {
                    if (interaction[prop]) {
            ...
            return event.targets
                .filter(target => target.interactive)
                .some(target => interactions.some(int => entityMatchesInteraction(target, int)));
        });
        const logActions = actions.map(el => el.name);
        logs.info("performAction", `${actions.length} actions by this interaction:`, logActions);
        return actions;
    }
    /**
     * Checks all attatched conditions (if any) to see if this action is legal
     */
    isLegal(conditions, event) {
