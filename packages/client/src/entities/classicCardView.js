...
import { cm2px } from "../../shared/numbers";
...
export class ClassicCardView extends EntityView {
    constructor(data) {
        ...
        this.paper = new Graphics()
            ...
            .drawRoundedRect(-ClassicCardView.width / 2, -ClassicCardView.height / 2, ClassicCardView.width, ClassicCardView.height, 8);
        // Back side graphics padding from paper's edge
        const BACK_PAD = 14;
        const BACK_COLOR = 0xb0342f;
        this.back = new Graphics()
            .beginFill(BACK_COLOR, 1)
            .drawRect(-ClassicCardView.width / 2 + BACK_PAD, -ClassicCardView.height / 2 + BACK_PAD, ClassicCardView.width - BACK_PAD * 2, ClassicCardView.height - BACK_PAD * 2)
            .endFill()
            .lineStyle(1, BACK_COLOR)
            .drawRect(-ClassicCardView.width / 2 + BACK_PAD - 2, -ClassicCardView.height / 2 + BACK_PAD - 2, ClassicCardView.width - BACK_PAD * 2 + 4, ClassicCardView.height - BACK_PAD * 2 + 4);
        this.rank = new Text(getRankText(this.data.rank), getRankStyle(this.data.rank));
        this.rank.x = -ClassicCardView.width / 2 + 5;
        this.rank.y = -ClassicCardView.height / 2 + 5;
        this.suit = new Text(getSuitText(this.data.suit), getSuitStyle(this.data.suit));
        this.suit.x = -ClassicCardView.width / 2 + 5;
        this.suit.y = -ClassicCardView.height / 2 - 5 + this.rank.height;
        this._idx = new Text(`(${this.data.idx})`, new TextStyle({
            fill: "#000000",
            fontSize: 12
        }));
...
ClassicCardView.width = cm2px(6.35);
ClassicCardView.height = cm2px(8.89);
const getRankText = (rank) => {
    return rank;
};
const getSuitText = (suit) => {
    switch (suit) {
        case "D":
            return "♦";
        case "C":
...
//# sourceMappingURL=classicCardView.js.map